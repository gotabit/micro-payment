...
pub fn verify_proof(pub_inputs: Vec<(impl ToString, impl Into<BigInt>)>) -> Result<()> {
    let mut builder = CircomBuilder::new(CIRCOM_CONFIG.to_owned());
    ...
    let circom: ark_circom::CircomCircuit<ark_ec::bn::Bn<ark_bn254::Config>> = builder.build()?;
